                    ...
                    && self.cursor_modifiers.control_key()
                {
                    ...
                    if key == protocol::keyboard_input::Key::Unknown {
                        ...
                        self.attachment
                            ...
                            .context("sending keyboard input")?;
                    }
                }
            }
...
pub fn main() -> anyhow::Result<()> {
    ...
    let client = client::Client::new(&args.host, "mmclient", DEFAULT_TIMEOUT).block_on()?;
...
fn init_window(
    ...
    let session_id = if let Some(session) = session {
        ...
        client
            .launch_session(target.clone(), desired_params.clone(), DEFAULT_TIMEOUT)
            .block_on()?
            .id
    };
