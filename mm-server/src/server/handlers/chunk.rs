...
pub fn iter_chunks(
    ...
    if let Some(ratio) = fec_ratio {
        return Either::Left(iter_chunks_fec(buf, mtu, ratio));
    }
...
mod tests {
    ...
    fn test_iter_chunks() {
        ...
        let mut chunks = iter_chunks(frame, 1200, None);
    ...
    fn test_iter_chunks_fec() {
        let frame = bytes::Bytes::from(vec![9; 3536]);
        let mut chunks = iter_chunks(frame, 1200, Some(0.15));
        let chunk = chunks.next().unwrap();
        assert_eq!(chunk.index, 0);
